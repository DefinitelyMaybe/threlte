...
The [`<GLTF/>`](/reference/extras/gltf) component is roughly equivalent to
the example above.

### `useGltf` hook

`@threlte/extras` provides a handy hook for loading `.gltf` models called
[useGltf](/reference/extras/use-gltf):
...
[useGltfAnimations](/reference/extras/use-gltf-animations) hook for gltfs.
