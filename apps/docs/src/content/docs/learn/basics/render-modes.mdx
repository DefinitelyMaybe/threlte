...
function from the [`useThrelte`](/reference/core/hooks/use-threlte) hook.
...
The [`useTask`](/reference/core/hooks/use-task) hook is by default configured to
...
[`useThrelte`](/reference/core/hooks/use-threlte) hook and set the `autoInvalidate`
option to `false`:
...
[`<Canvas>`](/reference/core/components/canvas) component:
...
[`useThrelte`](/reference/core/hooks/use-threlte) hook:
...
you may want to render the scene yourself, for example when using [postprocessing](/examples/visual-effects/outlines/).

1. Set `autoRender` to `false` on the [`<Canvas>`](/reference/core/components/canvas)
...
- Add a task to [Threlte's default
  `renderStage`](/learn/basics/scheduling-tasks#default-stages). Tasks in
...
- Use `shouldRender` from the hook
  [`useThrelte`](/reference/core/hooks/use-threlte). This function will evaluate
  ...
  `renderStage`](/learn/basics/scheduling-tasks#default-stages).
