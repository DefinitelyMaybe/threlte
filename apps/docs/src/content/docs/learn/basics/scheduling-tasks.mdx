...
Every Threlte app has a single scheduler. It is accessible via [`useThrelte()`](/reference/core/hooks/use-threlte):
...
- **`autoRenderTask`**: This task is part of [Threlte's
  ...
  [`autoRender`](/reference/core/components/canvas) is set to `true`.
