...
[Automatic disposal](/learn/basics/disposing-objects) has been improved to
...
For more information, see the [`useGltf`](/reference/extras/use-gltf) and [`GLTF`](/reference/extras/gltf) docs.
...
[framerate](/reference/rapier/framerate), i.e. the delta provided in these
...
event listener is placed. Check out [its documentation](/reference/extras/interactivity) to learn more.

### `useLoader` now returns a store

The hook [`useLoader`](/reference/core/hooks#useloader) now returns a custom Svelte store called
[`AsyncWritable`](/reference/core/utilities#asyncwritable). This store allows you to [await](https://svelte.dev/tutorial/await-blocks)
...
`EffectComposer` have been removed. Threlte 6 now provides a hook called [`useRender`](/reference/core/hooks#userender) which
...
[`CurrentWritable`](/reference/core/utilities#currentwritable) which is a store that contains a `current` value with
...
[`useFrame`](/reference/core/hooks#useframe) where the context is available as the first argument
to the callback:
...
The full type definition is [currently listed here](/reference/core/hooks#usethrelte).
...
Check out the [hooks documentation](/reference/extras/use-gltf-animations) for more information.
