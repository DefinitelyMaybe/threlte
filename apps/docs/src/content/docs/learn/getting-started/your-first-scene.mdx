...
First we create a new Svelte file, `App.svelte`, where we import [`<Canvas>`](/reference/core/components/canvas/).
...
and provides Threlte's runtime context. To access this runtime context, we'll need to [create a separate component](/learn/basics/app-structure)
...
In `Scene.svelte`, we import the [`<T>` component](/reference/core/components/t/), which is
...
based on the [Three.js docs](https://threejs.org/docs/index.html#manual/en/introduction/Creating-a-scene) for the class you are using.
...
Threlte supports listening to certain [events on `<T/>` components](/reference/core/components/t#events). Here, we use the `create`
...
[`interactivity`](/reference/extras/interaction/interactivity) [plugin](/learn/advanced/plugins/) from
[`@threlte/extras`](/reference/extras/getting-started) and invoke it in our `Scene.svelte` file.
...
Let's add some motion to our cube. We will use Threlte's [`useTask`](/reference/core/hooks/use-task) hook to tap
