...
Threlte's camera is a [currentWritable](/reference/core/helpers/utilities/#currentwritable) so we can use its **.current** property to get the instance. Its important to use the **current** property to avoid having to read from the store every frame.
