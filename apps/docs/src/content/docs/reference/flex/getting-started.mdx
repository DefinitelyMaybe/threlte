...
Use the component [`<Flex>`](/reference/flex/components/flex) to create a flexbox
container. Since there's no viewport to fill, you must specify the size of the
container. Add flex items with the component [`<Box>`](/reference/flex/components/box/).
...
The component [`<Align>`](/reference/extras/staging/align/) can be used to align the resulting flex container.
...
[`createClassParser`](/reference/flex/utilities/create-class-parser/) which accepts a
...
class names](/reference/flex/utilities/tailwind-parser/).
