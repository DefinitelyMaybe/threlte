...
child [`<Box>`](/reference/flex/components/box/) components to resolve their layout.
...
method [`createClassParser`](/reference/flex/utilities/create-class-parser/) or by
using a parser provided, for instance the Tailwind CSS parser
[`tailwindParser`](/reference/flex/utilities/tailwind-parser/).
...
node tree. If a node (i.e. a [`<Box>`](/reference/flex/components/box/) component) is added or removed
...
You may also use the hook [`useReflow`](/reference/flex/hooks/use-reflow/) to request
a reflow.

### Reflow Stage

By default, the reflow of the layout is happening in [Threlte's
mainStage](/learn/basics/scheduling-tasks#default-stages). To change in
...
- Using the hook [`useDimensions`](/reference/flex/hooks/use-dimensions/)
