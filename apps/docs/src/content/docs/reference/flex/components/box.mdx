...
of [`<Flex>`](/reference/flex/components/flex) or as a child of another `<Box>` to
...
- Using the [`useDimensions`](/reference/flex/hooks/use-dimensions/) hook in a child component to `<Box>`:
...
To trigger a [layout reflow](/reference/flex/components/flex#layout-reflow), you can use the `reflow` slot prop:
