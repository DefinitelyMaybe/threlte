...
<Tip type="info">
  ...
  ability](/learn/basics/scheduling-tasks#creating-a-stage) to run the tasks of a stage multiple
  times per frame and with a fixed delta.
</Tip>
...
Use the hook [`usePhysicsTask`](/reference/rapier/hooks/use-physics-task/) to
interact with the physics simulation. It's a wrapper around the [`useTask`
hook](/reference/core/hooks/use-task) that automatically adds the handler to the
