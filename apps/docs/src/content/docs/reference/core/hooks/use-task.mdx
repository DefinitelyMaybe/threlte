...
<Tip type="info">
  ...
  see the [scheduling tasks](/learn/basics/scheduling-tasks) page.
</Tip>

The hook `useTask` is used to create a
[task](/learn/basics/scheduling-tasks#tasks) that is used to run code on
every frame.
...
`mainStage`](/learn/basics/scheduling-tasks/#default-stages) in an arbitrary order (i.e. without dependencies).
...
added to [Threlte's
`mainStage`](/learn/basics/scheduling-tasks#default-stages).
...
<Tip type="warning">
  ...
  `mainStage`](/learn/basics/scheduling-tasks#default-stages), you will also need to pass the stage,
  either by value or key.{' '}
</Tip>
...
[`renderStage`](/learn/basics/scheduling-tasks#default-stages) which by
