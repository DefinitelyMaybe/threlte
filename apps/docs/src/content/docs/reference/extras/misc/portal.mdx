...
In this case, the portal target is easily accessible, and passing the `scene` object to the `<T>` component's [attach](/reference/core/components/t#attach) property can accomplish everything we need.
...
You can define where a `<Portal>` should render its children by using the component [`<PortalTarget>`](/reference/extras/portal-target).
