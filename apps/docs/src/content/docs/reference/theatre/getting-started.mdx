...
<Tip type="tip">
  The component `<Theatre>` provides a default [`<Project>`](/reference/theatre/components/project/) and [`<Sheet>`](/reference/theatre/components/sheet/) and implements [`<Studio>`](/reference/theatre/components/studio/). For a more flexible structure please consider using `<Project>`, `<Sheet>` and `<Studio>` on their own.
</Tip>
