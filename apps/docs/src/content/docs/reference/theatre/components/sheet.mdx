...
Theatre.js sheets contain one or more Theatre.js objects and optionally a [`<Sequence>`](/reference/theatre/components/sequence) component that allows controlling the animation.
The animations can adjusted within the interface provided by the [`<Studio>`](/reference/theatre/components/studio/) component.
...
You can create a sheet by placing the component `<Sheet>` as a child of a [`<Project>`](/reference/theatre/components/project/) component. If a sheet with the given name already exists, it will represent the existing sheet instead of creating a new one.
...
The first way to control the sequence is using a reactive API with the [`<Sequence>`](/reference/theatre/components/sequence) component.
