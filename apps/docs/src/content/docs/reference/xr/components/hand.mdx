...
To trigger reactive changes based on whether hand input is or is not present, the `useXR` hook provides a [`currentWritable`](/reference/core/helpers/utilities/#currentwritable) store:
