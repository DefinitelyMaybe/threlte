...
To use the Three.js WebGPU renderer, import it and then initialize it within
your `<Canvas>`'s `createRenderer` prop. This will replace the default WebGL
renderer.
...
First set up a [WebGPU renderer](#webgpu), then update the `<T>` catalogue.
This lets you assign TSL shaders to your material's nodes. Afterwards, checkout
our [VFX](/docs/reference/vfx/getting-started) package for some ready made
shaders.

<Example path="shaders/slice/main" />

<small>
  The example above is an adaptation of
  [this](https://threejs.org/examples/?q=tsl#webgpu_tsl_angular_slicing) Three.js example.
</small>

### Updating the \<T> catalogue

The `<T>` component uses all the exports from `three` and will error by default
on things like `<T.MeshPhysicalNodeMaterial />`. You have a few options to
resolve this.
...
Options 2 and 3 will keep the bundle size of your application smaller but you'll
have to keep it updated as you go.

#### Common mistakes

If using option 2 or 3, don't forget to update the lighting classes. Shader's
can behave weirdly if the renderer is not using the `three/webgpu` lighting
classes.

```svelte title="App.svelte" {2,5}m
<script>
...
<Tip type="tip">
  An easy option for projects is to start with [option #1](#option-1) and then transition to the
  other options when bundle size becomes an issue or you need to ship to production.
</Tip>
...
`shadowNode`. The `outputNode` is set up in such a way that it discards any
...
in the `color` uniform. The material needs its `side` set to `THREE.DoubleSide`
otherwise three.js will cull them out if they are facing away from the camera,

Any fragment that is discarded in the shadowNode will not cast shadows.

### Updating uniforms

If your node uses uniforms, they can be declared in the `script` tag of the
component and updated via `$effect` or a callback.

For example, if your material uses elapsed time in a uniform, you can update
the uniform inside a `useTask` callback.

The material in the example below demonstrates two ways to update uniforms. The
`uTime` uniform is updated in `useTask` whereas `uIntensity` is updated in an
`$effect`.
