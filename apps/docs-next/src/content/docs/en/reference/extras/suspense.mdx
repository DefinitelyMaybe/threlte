...
:::tip[Suspense-ready]
_Suspense-ready_ means it has the ability to hit a `<Suspense>` boundary if there is any. If there's no `<Suspense>` boundary, the component will behave as usual.
:::
