...
:::note
Tasks are part of Threlte's _Task Scheduling System_. For details on how to use stages and tasks,
...
:::caution
{' '}
If a task is passed by reference to the `before` or `after` option, the task created by `useTask` will
automatically be added to the same stage as the task it depends on. If you pass a key instead and the
task you want to reference is **not** in [Threlte's `mainStage`](/docs/learn/basics/scheduling-tasks#default-stages),
you will also need to pass the stage, either by value or key.{' '}
:::
